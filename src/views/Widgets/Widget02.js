...
class Widget02 extends Component {
  render() {
    const { className, cssModule, header, mainText, icon, color, footer, link, children, variant,onClick, buttonClassName,buttonColor, buttonStyle,buttonText, ...attributes } = this.props;
      ...
      <Card>
        <CardBody className={card.classes} {...attributes}>
          ...
          <div className="d-flex justify-content-between align-items-center">
            <span className="text-muted text-uppercase font-weight-bold font-xs">{mainText}</span>
            {onClick && <Button onClick={onClick} className={buttonClassName} color={buttonColor} style={buttonStyle}>{buttonText}</Button>}
          </div>
        </CardBody>
        {cardFooter()}
      </Card>
